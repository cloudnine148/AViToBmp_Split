...
BOOL CvideoDlg::OnInitDialog()
{
	...
	//////////////////////////////////
	CStatic* staticSize = (CStatic*)GetDlgItem(IDC_STATIC);
	CRect rect;
...
void CvideoDlg::processImage()
{
	...
	// main 함수 호출
	m_proc.main(m_Dib, m_nCurrentFrame);
	...
	CString temp;
	//처리시간. 단위 ms
	temp.Format(_T("%.1lf"),((double)(Endtime.QuadPart - BeginTime.QuadPart)/(double)Frequency.QuadPart)*1000);
	eb_processtime = temp;	
}
...
{
	CDC* pDC = GetDC();
	m_Dib.Draw(pDC->m_hDC,left,top,width,height);
}