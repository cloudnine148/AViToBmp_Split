...
*/
void COpticalFlow::Func_Init(int h, int w, int gridInterval, double imgReseRatio, double rateRoiH)
{	
	int imgH, imgW;	
	int gridCnt;
	...
	// 영상 관련 설정
	imgH = int(h * m_dResizeRatio);
	imgW = int(w * m_dResizeRatio);
	m_nImgHeight = imgH;
	m_nImgWidth = imgW;
	...
	else					m_nROIHeight = int(m_nImgHeight * rateRoiH);	
...
*/
int COpticalFlow::mainProcessing(IplImage* currImg, int nCurrentFrameNum)
{
	if( !m_bSetPrevImg )		
	{
...
{	
    ...
    { 
		...
		// 20170203 KJY
		if ( arrow_length < 3.5 || arrow_length > 40)
		{
			line_color = CV_RGB(100, 100, 100);		// gray
			continue;
		} 
		else
		{
			line_color = CV_RGB(0, 255, 0);			// green
		}
...
int COpticalFlow::SetOPValue(char* status, float* track_error, int nFeatureSize, CvPoint2D32f* prev_features, CvPoint2D32f* curr_features)
{
	...
	for( int i=0 ; i<nFeatureSize ; i++ )
	{
		if(status[i] != 0)
		{
			...
			m_op[valid_pt].magnitude = int(arrow_length);
...
{	
	...
	int op_mag = int(m_dVehicleMagnitudeResult / m_dResizeRatio);	// 원본 영상사이즈와 op 추출 영상 사이즈를 고려하여 optical flow magnitude 계산 
	...
	if( m_dVehicleMagnitudeResult > 3. && ( abs(angle180) >= 55.0 && abs(angle180) <= 115.0 ) )	// large moving
	{
		...
		roi_start_y = int(m_nPrevRoiStart_y - op_mag*sin(m_dVehicleAngleResult)); 
		m_nNonMovingCount = 0;
...
void COpticalFlow::print_ROI(IplImage* img, const char* show_name)
{
	...
	// 기본 ROI
	int droi_start_x = int(img->width * ROI_START_RATIO_X);
	int droi_start_y = int(img->height * ROI_START_RATIO_Y);
	int roi_start_x = int(m_nCurrRoiStart_x *  m_dResizeRatio);
	int roi_start_y = int(m_nCurrRoiStart_y *  m_dResizeRatio);
    ...
    CvFont font;
	int angle180 = 0;
	if( m_dVehicleAngleResult == 0.0 ) 		angle180 = 0;
...
void COpticalFlow::GetOpticalFlow(IplImage* preImg, IplImage* currImg, char* status, float* track_error, int nGridCount, CvPoint2D32f* prev_features, CvPoint2D32f* curr_features)
{
	...
	*/
	...
	/// ego motion 보정
	// 현재 입력 영상 IplImage 형 변환
	double rate = 0.982; //0.982

	int box_inter = (int)(5 * rate);

	int inter_x = (int)((m_nImgWidth - (int)(m_nImgWidth * rate)) * 0.5);
	int inter_y = (int)((m_nImgHeight - (int)(m_nImgHeight * rate)) * 0.5);;

	IplImage* temp_currImg;
	temp_currImg = cvCreateImage(cvSize(m_nImgWidth, m_nImgHeight), IPL_DEPTH_8U, 1);

	IplImage* re_currImg;
	re_currImg = cvCreateImage(cvSize((int)(m_nImgWidth * rate), (int)(m_nImgHeight * rate)), IPL_DEPTH_8U, 1);

	cvCopy(currImg, temp_currImg);

	cvResize(temp_currImg, re_currImg, 1);

	for (int i = 0; i < m_nImgHeight; i++)
	{
		for (int j = 0; j < m_nImgWidth; j++)
		{
			currImg->imageData[j + i * currImg->widthStep] = 0;
		}
	}

	for (int i = 0; i < (int)(m_nImgHeight * rate); i++)
	{
		for (int j = 0; j < (int)(m_nImgWidth * rate); j++)
		{
			currImg->imageData[(j + inter_x) + (i + inter_y) * currImg->widthStep] = re_currImg->imageData[j + i * re_currImg->widthStep];
		}
	}
	...
	/// 20170203 KJY
	cvReleaseImage(&re_currImg);
	cvReleaseImage(&temp_currImg);
...
/// 20170203 KJY
// optical flow map 생성
void COpticalFlow::getOpticalFlowMap(IplImage* img)
{
	register int i, j, k;
	int interval = int(m_nGridInterval * 0.5);
	double mag = 0.;

	int py, px;
	double radian = 0.;

	// optical flow map 초기화
	for (i = 0; i < img->height; i++)
	{
		for (j = 0; j < img->width; j++)
		{
			img->imageData[j + i * img->widthStep] = 0;
		}
	}

	// optical flow map 생성
	for (k = 0; k < m_nValidp_count; k++)
	{
		if (m_op[k].magnitude < 3.5 || m_op[k].magnitude > 40)
		{
			continue;
		}
		...
		{
			radian = m_op[k].angle;
			py = (int)(m_op[k].py - m_op[k].magnitude * sin(radian));
			px = (int)(m_op[k].px - m_op[k].magnitude * cos(radian));
		}

		for (i = py - interval; i < py + interval; i++)
		{
			for (j = px - interval; j < px + interval; j++)
			{
				...
				if (m_op[k].magnitude >= 10)
				{
					mag = 1.;
				}
				else
				{
					mag = m_op[k].magnitude * 0.1;
				}

				img->imageData[j + i * img->widthStep] = (BYTE)(mag * 255);
			}
		}
	}

	cvNamedWindow("Lkpyr_OpticalFlow_Map", CV_WINDOW_AUTOSIZE);
	cvShowImage("Lkpyr_OpticalFlow_Map", img);
}