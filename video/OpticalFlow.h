...
public:
	...
	int mainProcessing(IplImage* currImg, int nCurrentFrameNum);	
	
	// Optical Flow 함수 초기화 및 소멸
	void Func_Init(int imgH, int imgW, int gridInterval, double resizeRatio, double rateRoiH=-1.0);
	void Func_Destory();
	...
	/// 20170203 KJY
	// optical flow map 생성
	void getOpticalFlowMap(IplImage* img);
